...
router.get('/', async (req, res, next) => {
    try {
        const [results] = await dbConnection.promise().query('SELECT SUM(press_value) AS totalLikes FROM likeButtons;');
...
router.put('/increase', async (req, res, next) => {
    try {
        await dbConnection.promise().query('INSERT INTO likeButtons (press_value, timestamp_like) VALUES (1, CURRENT_TIMESTAMP);');
        db.likeCount++;
